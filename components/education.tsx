            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {certifications.map((cert, index) => (
                  <div
                    key={index}
                    className="p-3 bg-muted/10 dark:bg-white/5 rounded-lg hover:bg-muted/20 dark:hover:bg-white/10 transition-colors border-l-2 border-teal"
                  >
                    <h3 className="font-medium text-foreground dark:text-white text-sm">{cert.title}</h3>
                    <div className="flex justify-between items-center gap-2 mt-1">
                      <div className="flex items-center">
                        <span className="text-xs text-teal-light">{cert.provider}</span>
                        <Badge variant="outline" className="ml-2 border-pink-light text-pink-light text-xs px-1 py-0">
                          {cert.date}
                        </Badge>
                      </div>
                      <Button
                        size="sm"
                        variant="ghost"
                        className="h-7 px-2 text-xs text-purple-light hover:text-purple hover:bg-purple/10"
                        asChild
                      >
